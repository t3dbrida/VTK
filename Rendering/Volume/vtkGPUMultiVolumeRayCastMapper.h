...
=========================================================================*/
/**
 * @class   vtkGPUMultiVolumeRayCastMapper
...
#ifndef vtkGPUMultiVolumeRayCastMapper_h
#define vtkGPUMultiVolumeRayCastMapper_h

#define vtkSetIndexedObjectBodyMacro(name, type, args, i) \
{														  \
  if (this->name[i] != args)							  \
  {														  \
    type* tempSGMacroVar = this->name[i];                 \
    this->name[i] = args;                                 \
    if (this->name[i] != NULL)                            \
	{                                                     \
      this->name[i]->Register(this);                      \
	}                                                     \
...
#define vtkCxxSetIndexedObjectMacro(class, name, type) \
void class::Set##name (int i, type* arg)        	   \
{													   \
  vtkSetIndexedObjectBodyMacro(name, type, arg, i);    \
}

#define vtkCxxGetIndexedObjectMacro(name, type) \
type Get##name(int i)                   	    \
{												\
   return name[i];                              \
}
...
class VTKRENDERINGVOLUME_EXPORT vtkGPUMultiVolumeRayCastMapper : public vtkMultiVolumeMapper
{
public:
  static vtkGPUMultiVolumeRayCastMapper *New();
  vtkTypeMacro(vtkGPUMultiVolumeRayCastMapper,vtkMultiVolumeMapper);
   ...
   */
  void Render( vtkRenderer *, vtkMultiVolume2 *) VTK_OVERRIDE;

  /**
   * Handled in the subclass - the actual render method
   * \pre input is up-to-date.
   */
  virtual void GPURender( vtkRenderer *, vtkMultiVolume2 *) {}
...
protected:
  ...
  ~vtkGPUMultiVolumeRayCastMapper() VTK_OVERRIDE;

  // Check to see that the render will be OK
  int ValidateRender( vtkRenderer *, vtkMultiVolume2 * );


  // Special version of render called during the creation
  // of a canonical view.
  void CanonicalViewRender( vtkRenderer *, vtkMultiVolume2 * );
  ...
  std::vector<vtkImageData*> TransformedInput;

  //vtkCxxGetIndexedObjectMacro(TransformedInput, vtkImageData*);
  vtkImageData* GetTransformedInput(int i);
  vtkImageData* SetLastInput(int i, vtkImageData*);
  void SetTransformedInput(int i , vtkImageData* in)
  {
	  while (this->TransformedInput.size() <= i && i < this->GetNumberOfInputConnections(0))
		  TransformedInput.push_back(nullptr);

	  this->TransformedInput[i] = in;
  }
   ...
   * Render() call.
   */
  std::vector<vtkImageData*> LastInput;

  vtkImageData* GetLastInput(int i);

private:

  vtkGPUMultiVolumeRayCastMapper(const vtkGPUMultiVolumeRayCastMapper&) VTK_DELETE_FUNCTION;
  void operator=(const vtkGPUMultiVolumeRayCastMapper&) VTK_DELETE_FUNCTION;
};
