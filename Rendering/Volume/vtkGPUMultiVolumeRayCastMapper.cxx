...
//vtkCxxSetIndexedObjectMacro(vtkGPUMultiVolumeRayCastMapper, TransformedInput, vtkImageData);
...
//   - Invoke an end event
// ----------------------------------------------------------------------------
void vtkGPUMultiVolumeRayCastMapper::Render( vtkRenderer *ren, vtkMultiVolume2 *vols )
{
  // Catch renders that are happening due to a canonical view render and
  // handle them separately.
  if (this->GeneratingCanonicalView )
  {
    this->CanonicalViewRender(ren, vols);
    return;
  }
  ...
  if ( this->ValidateRender(ren, vols ) )
  {
    // Everything is OK - so go ahead and really do the render
    this->GPURender( ren, vols);
  }
  ...
  if ( vols->GetAllocatedRenderTime() < 1.0 )
  {
                                                   ...
                                                   vtkMultiVolume2 *vol)
{
  ...
  {
    goodSoFar = 0;
    vtkErrorMacro("Volume list cannot be null.");
  }
...
//  }
  ...
  if (goodSoFar) {
	  for (int i = 0; i < this->GetNumberOfInputConnections(0); ++i)
		  this->GetInputAlgorithm(0, i)->Update();
  }

  for (int i = 0; i < this->GetNumberOfInputConnections(0) && goodSoFar; ++i) {
	  if ((this->GetInput(i) != this->GetLastInput(i) ||
		  this->GetInput(i)->GetMTime() > this->GetTransformedInput(i)->GetMTime()))
	  {
		  this->SetLastInput(i, this->GetInput(i));

		  vtkImageData* clone;
		  if (!this->GetTransformedInput(i)) {
			  clone = vtkImageData::New();
			  this->SetTransformedInput(i, clone);
			  //clone->Delete();
		  }
		  else {
			  clone = this->GetTransformedInput(i);
		  }

		  clone->ShallowCopy(this->GetInput(i));
  ...
  if ( goodSoFar )
  {
	vtkDataArray *scalars = NULL;
	  ...
	  for (int i = 0; i < this->GetNumberOfInputConnections(0); ++i) {
		  ...
		  scalars = this->GetScalars(this->TransformedInput[i],
			  this->ScalarMode,
  ...
  {
	  for (int i = 0; i < this->GetNumberOfInputConnections(0); ++i) {
		  vtkDataArray* scalars = this->GetScalars(this->TransformedInput[i],
			  this->ScalarMode,
			  this->ArrayAccessMode,
			  this->ArrayId,
			  this->ArrayName,
  ...
  if ( goodSoFar )
  {
    if (this->BlendMode != vtkMultiVolumeMapper::COMPOSITE_BLEND &&
        this->BlendMode != vtkMultiVolumeMapper::MAXIMUM_INTENSITY_BLEND &&
        this->BlendMode != vtkMultiVolumeMapper::MINIMUM_INTENSITY_BLEND &&
        this->BlendMode != vtkMultiVolumeMapper::AVERAGE_INTENSITY_BLEND &&
        this->BlendMode != vtkMultiVolumeMapper::ADDITIVE_BLEND)
    {
      goodSoFar = 0;
  ...
  vtkDataArray* scalars = this->GetScalars(this->TransformedInput[0],
	  this->ScalarMode,
	  this->ArrayAccessMode,
	  this->ArrayId,
	  this->ArrayName,
  ...
  if( goodSoFar )
  {
    if( !(vol->GetProperty(0)->GetIndependentComponents()) &&
...
//    }
  ...
  double volBounds[6];
  //this->GetInput()->GetBounds(volBounds); // XXX
  this->GetBounds(volBounds);
...
//----------------------------------------------------------------------------
vtkContourValues* vtkGPUMultiVolumeRayCastMapper::GetDepthPassContourValues()
{
  if (!this->DepthPassContourValues)
  {
...
//----------------------------------------------------------------------------
void vtkGPUMultiVolumeRayCastMapper::SetDepthImageScalarTypeToUnsignedChar()
{
  this->SetDepthImageScalarType(VTK_UNSIGNED_CHAR);
}

//----------------------------------------------------------------------------
void vtkGPUMultiVolumeRayCastMapper::SetDepthImageScalarTypeToUnsignedShort()
{
  this->SetDepthImageScalarType(VTK_UNSIGNED_SHORT);
}

//----------------------------------------------------------------------------
void vtkGPUMultiVolumeRayCastMapper::SetDepthImageScalarTypeToFloat()
{
  this->SetDepthImageScalarType(VTK_FLOAT);
}

//----------------------------------------------------------------------------
vtkImageData* vtkGPUMultiVolumeRayCastMapper::GetTransformedInput(int i)
{
	if (i < 0)
		return nullptr;

	while (i >= this->TransformedInput.size() && i < this->GetNumberOfInputConnections(0))
		this->TransformedInput.push_back(nullptr);
	return this->TransformedInput[i];
}

//----------------------------------------------------------------------------
vtkImageData* vtkGPUMultiVolumeRayCastMapper::GetLastInput(int i)
{
	if (i < 0)
		return nullptr;

	while (i >= this->LastInput.size() && i < this->GetNumberOfInputConnections(0))
		this->LastInput.push_back(nullptr);

	return this->LastInput[i];
}

//----------------------------------------------------------------------------
vtkImageData* vtkGPUMultiVolumeRayCastMapper::SetLastInput(int i, vtkImageData* data)
{
	this->GetLastInput(i);
	this->LastInput[i] = data;
	return LastInput[i];
}