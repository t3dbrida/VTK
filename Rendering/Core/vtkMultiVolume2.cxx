...
vtkMultiVolume2::~vtkMultiVolume2()
{
  for (int i = 0; i < this->Property.size(); ++i)
	  if (this->Property[i])
		this->Property[i]->UnRegister(this);
...
void vtkMultiVolume2::GetVolumes(vtkPropCollection *vc)
{
  vc->AddItem(this);
}

// Shallow copy of an volume.
void vtkMultiVolume2::ShallowCopy(vtkProp *prop)
{
  vtkMultiVolume2 *v = vtkMultiVolume2::SafeDownCast(prop);

  if ( v != NULL )
  {
    this->SetMapper(v->GetMapper());
	for (int i = 0; this->Mapper->GetNumberOfInputConnections(0); ++i)
		this->SetProperty(i, v->GetProperty(i));
  }
...
/*float *vtkMultiVolume2::GetScalarOpacityArray(int index)
{
...
float *vtkMultiVolume2::GetCorrectedScalarOpacityArray(int index)
{
  if ( index < 0 || index >= VTK_MAX_VRCOMP )
  {
    vtkErrorMacro("Index out of range [0-" << VTK_MAX_VRCOMP <<
...
float *vtkMultiVolume2::GetGradientOpacityArray(int index)
{
  if ( index < 0 || index >= VTK_MAX_VRCOMP )
  {
    vtkErrorMacro("Index out of range [0-" << VTK_MAX_VRCOMP <<
...
float vtkMultiVolume2::GetGradientOpacityConstant(int index)
{
  if ( index < 0 || index >= VTK_MAX_VRCOMP )
  {
    vtkErrorMacro("Index out of range [0-" << VTK_MAX_VRCOMP <<
...
float *vtkMultiVolume2::GetGrayArray(int index)
{
  if ( index < 0 || index >= VTK_MAX_VRCOMP )
  {
    vtkErrorMacro("Index out of range [0-" << VTK_MAX_VRCOMP <<
...
float *vtkMultiVolume2::GetRGBArray(int index)
{
  if ( index < 0 || index >= VTK_MAX_VRCOMP )
  {
    vtkErrorMacro("Index out of range [0-" << VTK_MAX_VRCOMP <<
...
void vtkMultiVolume2::SetMapper(vtkAbstractMultiVolumeMapper *mapper)
{
...
double vtkMultiVolume2::ComputeScreenCoverage( vtkViewport *vp )
{
  double coverage = 1.0;
...
double *vtkMultiVolume2::GetBounds()
{
  int i, n;
  double bbox[24], *fptr;
...
double vtkMultiVolume2::GetMinXBound( )
{
  this->GetBounds();
...
double vtkMultiVolume2::GetMaxXBound( )
{
  this->GetBounds();
...
double vtkMultiVolume2::GetMinYBound( )
{
  this->GetBounds();
...
double vtkMultiVolume2::GetMaxYBound( )
{
  this->GetBounds();
...
double vtkMultiVolume2::GetMinZBound( )
{
  this->GetBounds();
...
double vtkMultiVolume2::GetMaxZBound( )
{
  this->GetBounds();
...
int vtkMultiVolume2::RenderVolumetricGeometry( vtkViewport *vp )
{
  this->Update();
  ...
  if ( !this->Mapper->GetNumberOfInputConnections(0) )
  {
    return 0;
  }

  // Force the creation of a property
  for (int i = this->Property.size(); i < this->Mapper->GetNumberOfInputConnections(0); ++i)
  {
	  if (!this->GetProperty(i))
	  {
...
void vtkMultiVolume2::ReleaseGraphicsResources(vtkWindow *win)
{
...
void vtkMultiVolume2::SetProperty(int i, vtkVolumeProperty *property)
{
  if (i < 0)
	return;

  if( this->GetProperty(i) != property)
  {
    ...
    if (this->Property[i] != NULL)
    {
      this->Property[i]->Register(this);
      this->Property[i]->UpdateMTimes();
    }
    this->Modified();
  }
}

vtkVolumeProperty *vtkMultiVolume2::GetProperty(int i)
{
  if (i < 0)
	return nullptr;

  while (i >= this->Property.size())
    this->Property.push_back(nullptr);

  if (!this->Property[i])
  {
    this->Property[i] = vtkVolumeProperty::New();
    this->Property[i]->Register(this);
    //this->Property[i]->Delete();
  }
  return this->Property[i];
}

vtkMTimeType vtkMultiVolume2::GetMTime()
{
  ...
  for (int i = 0; this->Mapper->GetNumberOfInputConnections(0); ++i)
  {
	  time = this->GetProperty(i)->GetMTime();
...
vtkMTimeType vtkMultiVolume2::GetRedrawMTime()
{
  ...
  if ( this->Mapper != NULL )
  {
    ...
    mTime = ( time > mTime ? time : mTime );
	for (int i = 0; i < this->Mapper->GetNumberOfInputConnections(0); ++i)
	{
		if (this->GetMapper()->GetDataSetInput(i) != NULL)
		{
			this->GetMapper()->GetInputAlgorithm()->Update();
			time = this->Mapper->GetDataSetInput(i)->GetMTime();
			mTime = (time > mTime ? time : mTime);
		}
    }
  }
  ...
  {
	  if (this->GetProperty(i)) {
		  time = this->Property[i]->GetMTime();
		  mTime = (time > mTime ? time : mTime);

		  int numComponents;

		  if (this->Mapper && this->Mapper->GetDataSetInput(i) &&
			  this->Mapper->GetDataSetInput(i)->GetPointData() &&
			  this->Mapper->GetDataSetInput(i)->GetPointData()->GetScalars())
		  {
			  numComponents = this->Mapper->GetDataSetInput(i)->GetPointData()->
		  ...
		  for (int j = 0; j < numComponents; j++)
		  {
			  ...
			  if (this->Property[i]->GetColorChannels(j) == 1)
			  {
				  time = this->Property[i]->GetGrayTransferFunction(j)->GetMTime();
				  mTime = (time > mTime ? time : mTime);
			  }
			  else
			  {
				  time = this->Property[i]->GetRGBTransferFunction(j)->GetMTime();
				  mTime = (time > mTime ? time : mTime);
			  }

			  // check the scalar opacity function
			  time = this->Property[i]->GetScalarOpacity(j)->GetMTime();
			  mTime = (time > mTime ? time : mTime);

			  // check the gradient opacity function
			  time = this->Property[i]->GetGradientOpacity(j)->GetMTime();
			  mTime = (time > mTime ? time : mTime);
		  }
	  }
  }
...
/*void vtkMultiVolume2::UpdateTransferFunctions( vtkRenderer *vtkNotUsed(ren) )
{
  int                        dataType;
...
/*void vtkMultiVolume2::UpdateScalarOpacityforSampleSize( vtkRenderer *vtkNotUsed(ren),
...
void vtkMultiVolume2::PrintSelf(ostream& os, vtkIndent indent)
{
  ...
  {
	  if (this->Property[i]) {
		  os << indent << "Property " << i << ":\n";
		  this->Property[i]->PrintSelf(os, indent.GetNextIndent());
	  }
	  else
		os << indent << "Property " << i << ": (not defined)\n";
  }
