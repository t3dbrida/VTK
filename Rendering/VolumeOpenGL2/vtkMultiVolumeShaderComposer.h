...
#ifndef vtkMultiVolumeShaderComposer_h
#define vtkMultiVolumeShaderComposer_h
...
namespace vtkmvolume
{
  ...
  std::string ComputeClipPositionImplementation(vtkRenderer* vtkNotUsed(ren),
                                                vtkMultiVolumeMapper* vtkNotUsed(mapper),
...
\n\
  ...
  std::string BaseDeclarationVertex(vtkRenderer* vtkNotUsed(ren),
                                    vtkMultiVolumeMapper* mapper,
                                    vtkMultiVolume2* vtkNotUsed(vol))
  {
	std::string num = std::to_string(mapper->GetNumberOfInputConnections(0));
    return std::string("\
      \n  uniform int in_instanceID;\
      \n  uniform bool in_cellFlag[" + num + "];\
      \n  uniform vec3 in_cellSpacing[" + num + "];\
      \n  uniform mat4 in_modelViewMatrix;\
      \n  uniform mat4 in_projectionMatrix;\
      \n  uniform mat4 in_volumeMatrix[" + num + "];\
      \n  uniform mat4 in_inverseVolumeMatrix[" + num + "];\
      \n\
      \n  uniform vec3 in_volumeExtentsMin[" + num + "];\
      \n  uniform vec3 in_volumeExtentsMax[" + num + "];\
      \n\
      \n  uniform mat4 in_inverseTextureDatasetMatrix[" + num + "];\
      \n  uniform mat4 in_cellToPoint[" + num + "];\
      \n\
      \n  //This variable could be 'invariant varying' but it is declared\
      \n  //as 'varying' to avoid compiler compatibility issues.\
      \n  varying vec3 ip_textureCoords[" + num + "];\
      \n  varying vec3 ip_vertexPos[" + num + "];\
      \n  varying mat4 ip_inverseTextureDataAdjusted[" + num + "];");
  }
  ...
  {
	std::string shaderStr = "// Volume dataset";

	std::string num = std::to_string(mapper->GetNumberOfInputConnections(0));

	shaderStr += "\nuniform sampler3D in_volume[" + num + "];";
	...
	shaderStr += std::string("\
	 ...
	 shaderStr += " \nuniform mat4 in_volumeMatrix[" + num + "];\
      ...
      \nuniform mat4 in_textureDatasetMatrix[" + num + "];\
      \nuniform mat4 in_inverseTextureDatasetMatrix[" + num + "];\
      \nvarying mat4 ip_inverseTextureDataAdjusted[" + num + "];\
      \nuniform vec3 in_texMin[" + num + "];\
      \nuniform vec3 in_texMax[" + num + "];\
      \nuniform mat4 in_textureToEye[" + num + "];\
      \nuniform vec3 in_volumeExtentsMin[" + num + "];\
      \nuniform vec3 in_volumeExtentsMax[" + num + "];\
      \nuniform mat4 in_cellToPoint[" + num + "];\
      \n\
      \n// Ray step size\
      \nuniform vec3 in_cellStep[" + num + "];\
      \nuniform vec2_4 in_scalarsRange[" + num + "];\
      \nuniform vec3 in_cellSpacing[" + num + "];\
      \n\
      \n// Sample distance\
      \nuniform float in_sampleDistance[" + num + "];\
      \n\
      \n// Scales\
      \nuniform vec3 in_cellScale[" + num + "];\
      \nuniform vec2 in_windowLowerLeftCorner;\
      \nuniform vec2 in_inverseOriginalWindowSize;\
      \nuniform vec2 in_inverseWindowSize;\
      \nuniform vec3 in_textureExtentsMin[" + num + "];\
      \nuniform vec3 in_textureExtentsMax[" + num + "];\
      \n\
      \n// Material and lighting";

	shaderStr += "\nuniform vec3_4 in_diffuse[" + num + "];\
			      \nuniform vec3_4 in_ambient[" + num + "];\
				  \nuniform vec3_4 in_specular[" + num + "];\
				  \nuniform float_4 in_shininess[" + num + "];";
    ...
    {
      shaderStr += std::string("\
        \nuniform float_4 in_componentWeight[" + num + "];");
    }

    vtkOpenGLGPUMultiVolumeRayCastMapper* glMapper
      = vtkOpenGLGPUMultiVolumeRayCastMapper::SafeDownCast(mapper);
    if (glMapper->GetCurrentPass() != vtkOpenGLGPUMultiVolumeRayCastMapper::DepthPass &&
                       ...
                       int lightingComplexity)
  {
	...
	shaderStr += "\n  for (int i = 0; i < " + std::to_string(glMapper->GetNumberOfInputConnections(0)) + "; ++i) {";

	//shaderStr += "\n    bool l_adjustTextureExtents =  !in_cellFlag[ip_instanceID];";
        ...
        vtkMultiVolumeMapper::COMPOSITE_BLEND)
    {
      shaderStr += std::string("\
        ...
        \n            in_inverseVolumeMatrix[ip_instanceID] *\
    ...
    //}
      ...
      shaderStr +=
        ...
        \n  g_eyePosObj[i] = (in_inverseVolumeMatrix[i] * vec4(in_cameraPos[i], 1.0));\
        \n  if (g_eyePosObj[i].w != 0.0)\
        \n  {\
        \n    g_eyePosObj[i].x /= g_eyePosObj[i].w;\
        \n    g_eyePosObj[i].y /= g_eyePosObj[i].w;\
        \n    g_eyePosObj[i].z /= g_eyePosObj[i].w;\
        \n    g_eyePosObj[i].w = 1.0;\
        \n  }\
        \n\
        \n  vec3 rayDir = computeRayDirection(i);\
        \n\
        \n  // Multiply the raymarching direction with the step size to get the\
        \n  // sub-step size we need to take at each raymarching step\
        \n  g_dirStep[i] = (ip_inverseTextureDataAdjusted[i] *\
        \n                  vec4(rayDir, 0.0)).xyz * in_sampleDistance[i];\
        \n\
	...
	if (vol->GetProperty(0)->GetShade() && lightingComplexity == 1)
	{
		shaderStr += "\
			  \n  // Light position in dataset space\
			  \n  g_lightPosObj[i] = (in_inverseVolumeMatrix[i] *\
			  \n                      vec4(in_cameraPos, 1.0));\
			  \n  if (g_lightPosObj[i].w != 0.0)\
			  \n  {\
			  \n    g_lightPosObj[i].x /= g_lightPosObj[i].w;\
			  \n    g_lightPosObj[i].y /= g_lightPosObj[i].w;\
			  \n    g_lightPosObj[i].z /= g_lightPosObj[i].w;\
			  \n    g_lightPosObj[i].w = 1.0;\
			  \n  }\
			  \n  g_ldir[i] = normalize(g_lightPosObj[i].xyz - ip_vertexPos[i]);\
			  \n  g_vdir[i] = normalize(g_eyePosObj[i].xyz - ip_vertexPos[i]);\
			  \n  g_h[i] = normalize(g_ldir[i] + g_vdir[i]);";
    }
	/*if ((vol->GetProperty(0)->GetShade() ||
		  vol->GetProperty(0)->HasGradientOpacity()) &&
		  glMapper->GetCurrentPass() != vtkOpenGLGPUMultiVolumeRayCastMapper::DepthPass)
	  {
		shaderStr += "\
					  \n  g_xvec[i] = vec3(in_cellStep[i].x, 0.0, 0.0);\
					  \n  g_yvec[i] = vec3(0.0, in_cellStep[i].y, 0.0);\
					  \n  g_zvec[i] = vec3(0.0, 0.0, in_cellStep[i].z);";
	  }*/
	 /* if (vol->GetProperty(0)->HasGradientOpacity())
	  {
		  shaderStr += std::string("\
	\n  g_cellSpacing = vec3(in_cellSpacing[ip_instanceID][0],\
	\n                       in_cellSpacing[ip_instanceID][1],\
	\n                       in_cellSpacing[ip_instanceID][2]);\
                                           ...
                                           gradientTableMap)
  {
    ...
    /*if ((noOfComponents == 1 || !independentComponents))
    {
	  for (int i = 0; i < mapper->GetNumberOfInputConnections(0); ++i)
		  if (vol->GetProperty(i)->HasGradientOpacity())
			  shaderStr += std::string("\
				\nuniform sampler2D " + gradientTableMap[i][0] + ";\
				\nfloat computeGradientOpacity_" + std::to_string(i) + " (vec4 grad)\
				\n{\
				\n  return texture2D(" + gradientTableMap[i][0] + ", vec2(grad.w, 0.0)).r;\
				\n}"
			  );
    }
	...
	//	if (vol->GetProperty(i)->GetShade() &&
	//		!vol->GetProperty(i)->HasGradientOpacity())
	//	{
			shaderStr += std::string("\
			\n// c is short for component\
			\nvec4 computeGradient(int i, int c)\
			\n{\
			\n  // Approximate Nabla(F) derivatives with central differences.\
			\n  vec3 g1; // F_front\
			\n  vec3 g2; // F_back\
			\n  g1.x = texture3D(in_volume[i], vec3(g_dataPos[i] + g_xvec[i]))[c];\
			\n  g1.y = texture3D(in_volume[i], vec3(g_dataPos[i] + g_yvec[i]))[c];\
			\n  g1.z = texture3D(in_volume[i], vec3(g_dataPos[i] + g_zvec[i]))[c];\
			\n  g2.x = texture3D(in_volume[i], vec3(g_dataPos[i] - g_xvec[i]))[c];\
			\n  g2.y = texture3D(in_volume[i], vec3(g_dataPos[i] - g_yvec[i]))[c];\
			\n  g2.z = texture3D(in_volume[i], vec3(g_dataPos[i] - g_zvec[i]))[c];\
			\n\
			\n  // Apply scale and bias to the fetched values.\
			\n  g1 = g1 * in_volume_scale[i][c] + in_volume_bias[i][c];\
			\n  g2 = g2 * in_volume_scale[i][c] + in_volume_bias[i][c];\
			\n\
		...
		//}
		/*else if (vol->GetProperty(i)->HasGradientOpacity())
		{
			shaderStr += "\
			\n// c is short for component\
			\nvec4 computeGradient(int i, int c)\
		...
		/*{
			shaderStr += "\
			\nvec4 computeGradient(int i, int component)\
			\n{\
			\n  return vec4(0.0);\
			\n}";
		}*/
                                         ...
                                         int lightingComplexity)
  {
	  ...
	  for (int i = 0; i < mapper->GetNumberOfInputConnections(0); ++i)
	  {
		  vtkVolumeProperty* volProperty = vol->GetProperty(i);
		  shaderStr = "\
		  \nvec4 computeLighting(int i, vec4 color, int component)\
		  \n  {\
		  ...
		  if (shadeReqd || volProperty->HasGradientOpacity())
		  {
			  //for (int i = 0; i < mapper->GetNumberOfInputConnections(0); ++i)
			  shaderStr += std::string("\
				\n  vec4 gradient = computeGradient(i, component);"
			  );
		  }
		  ...
		  if (shadeReqd)
		  {
			  ...
			  \n  vec3 normal = gradient.xyz / in_cellSpacing[i];\
			  ...
			  \n  }\
			  \n  float nDotL = dot(normal, g_ldir[i]);\
			  \n  float nDotH = dot(normal, g_h[i]);\
			  ...
			  \n  if (nDotL > 0.0)\
			  \n  {\
			  \n     diffuse = nDotL * in_diffuse[i].data[component] *\
			  \n               in_lightDiffuseColor[0] * color.rgb;\
			  \n  }\
			  \n  specular = pow(nDotH, in_shininess[i].data[component]) *\
			  \n                 in_specular[i].data[component] *\
			  \n                 in_lightSpecularColor[0];\
			  \n  // For the headlight, ignore the light's ambient color\
			  \n  // for now as it is causing the old mapper tests to fail\
			  \n  finalColor.xyz = in_ambient[i].data[component] * color.rgb +\
			  \n                   diffuse + specular;"
				  );
			  }
			  /*else if (lightingComplexity == 2)
			  ...
			  else if (lightingComplexity == 3)
			  {
				shaderStr += std::string("\
				  ...
				  \n  vec3 normal = normalize((in_textureToEye * vec4(gradient_0.xyz, 0.0)).xyz);\
				  \n  vec3 lightDir;\
				  \n  for (int lightNum = 0; lightNum < in_numberOfLights; lightNum++)\
				  \n    {\
		  ...
		  {
			  shaderStr += std::string("\
			\n  if (gradient_" + std::to_string(i) + ".w >= 0.0)\
			\n    {\
			\n    color.a = color.a *\
			\n              computeGradientOpacity_" + std::to_string(i) + "(gradient_" + std::to_string(i) + ");\
			\n    }"
			  );
		  }*/
		  /*else if (noOfComponents > 1 && independentComponents &&
                                             ...
                                             int vtkNotUsed(noOfComponents))
  {
    ...
    /*else
    {
      return std::string("\
        ...
        \nvec3 computeRayDirection()\
        \n{\
        \n  return normalize((in_inverseVolumeMatrix[i] *\
  ...
  std::string ComputeColorDeclaration(vtkRenderer* vtkNotUsed(ren),
                                      ...
                                      int noOfComponents,
                                      int independentComponents,
                                      std::map<int, std::map<int, std::string>> colorTableMap)
  {
      if (noOfComponents == 1)
      {
		std::string str = "";
		//for (int i = 0; i < mapper->GetNumberOfInputConnections(0); ++i) {
			str += "\nuniform sampler2D in_colorTransferFunc_0[" + std::to_string(mapper->GetNumberOfInputConnections(0)) + "];";
			str += "\nvec4 computeColor(int i, vec4 scalar, float opacity)\
			  \n{\
			  \n	return computeLighting(i, vec4(texture2D(in_colorTransferFunc_0[i],\
  ...
  std::string ComputeOpacityDeclaration(vtkRenderer* vtkNotUsed(ren),
                                        ...
                                        int noOfComponents,
                                        int independentComponents,
                                        std::map<int, std::map<int, std::string>> opacityTableMap)
  {
    /*if (noOfComponents > 1 && independentComponents)
    {
      std::string shaderStr;
    ...
    else*/
    {
		  std::string str = "";
		  //for (int i = 0; i < mapper->GetNumberOfInputConnections(0); ++i)
			str += "\nuniform sampler2D in_opacityTransferFunc_0[" + std::to_string(mapper->GetNumberOfInputConnections(0)) + "];\
					\nfloat computeOpacity(int i, vec4 scalar)\
					\n  {\
					\n  return texture2D(in_opacityTransferFunc_0[i], vec2(scalar.w, 0)).r;\
					\n  }";
		  return str;
      /*return std::string("\
        \nuniform sampler2D in_opacityTransferFunc;\
  ...
  std::string ShadingDeclarationFragment(vtkRenderer* vtkNotUsed(ren),
                                         ...
                                         vtkMultiVolume2* vtkNotUsed(vol))
  {
    if (mapper->GetBlendMode() == vtkMultiVolumeMapper::MAXIMUM_INTENSITY_BLEND)
    {
      return std::string("\
        \n bool l_firstValue;\
        \n vec4 l_maxValue;");
             ...
             vtkMultiVolumeMapper::MINIMUM_INTENSITY_BLEND)
    {
      return std::string("\
        \n bool l_firstValue;\
        \n vec4 l_minValue;");
    }
    else if (mapper->GetBlendMode() == vtkMultiVolumeMapper::AVERAGE_INTENSITY_BLEND)
    {
      return std::string("\
        \n  uvec4 l_numSamples;\
        \n  vec4 l_avgValue;");
    }
    else if (mapper->GetBlendMode() == vtkMultiVolumeMapper::ADDITIVE_BLEND)
    {
      return std::string("\
        \n  vec4 l_sumValue;");
    }
                                    ...
                                    int independentComponents = 0)
  {
    vtkOpenGLGPUMultiVolumeRayCastMapper* glMapper =
      vtkOpenGLGPUMultiVolumeRayCastMapper::SafeDownCast(mapper);
	std::string shaderStr = "\
       \n   if(any(greaterThan(g_dataPos[i], in_texMax[i])) ||\
       \n      any(lessThan(g_dataPos[i], in_texMin[i])) || !g_use[i])\
	   \n   {\
	   \n	   continue;\
	   \n   }";
    ...
    //if (mapper->GetBlendMode() == vtkMultiVolumeMapper::MAXIMUM_INTENSITY_BLEND)
    {
      ...
      {
        shaderStr += std::string("\
          \n      if (l_maxValue.w < scalar_" + std::to_string(i) + ".x || l_firstValue)\
          \n      {\
          \n        l_maxValue.w = scalar_" + std::to_string(i) + ".x;\
          \n      }\
          \n\
          \n      if (l_firstValue)\
          \n      {\
    ...
    else if (mapper->GetBlendMode() == vtkMultiVolumeMapper::MINIMUM_INTENSITY_BLEND)
    {
      /*if (noOfComponents > 1)
      {
        if (!independentComponents)
      ...
      else
      {
		for (int i = 0; i < mapper->GetNumberOfInputConnections(0); ++i)
        shaderStr += std::string("\
          \n      if (l_minValue.w > scalar_" + std::to_string(i) + ".x || l_firstValue)\
          \n      {\
          \n        l_minValue.w = scalar_" + std::to_string(i) + ".x;\
          \n      }\
          \n\
          \n      if (l_firstValue)\
          \n      {\
    ...
    //else if (mapper->GetBlendMode() == vtkMultiVolumeMapper::AVERAGE_INTENSITY_BLEND)
    {
      ...
      /*{
        ...
        \n                        (in_scalarsRange[0][1] -\
        \n                         in_scalarsRange[0][0]) * scalar_" + std::to_string(i) + ".x;\
        \n      if (in_averageIPRange.x <= intensity &&\
        \n          intensity <= in_averageIPRange.y)\
        \n      {\
        \n        l_avgValue.x += computeOpacity_" + std::to_string(i) +
		                          "(scalar_" + std::to_string(i) + ") * scalar_" +
								  std::to_string(i) + ".x;\
        \n        ++l_numSamples.x;\
        \n      }"
        );
    ...
    /*else if (mapper->GetBlendMode() == vtkMultiVolumeMapper::ADDITIVE_BLEND)
    {
      ...
      else
      {
		for (int i = 0; i < mapper->GetNumberOfInputConnections(0); ++i)
        shaderStr += std::string("\
          \n      float opacity_" + std::to_string(i) + " = computeOpacity_" + std::to_string(i) + "(scalar_" + std::to_string(i) + ");\
          \n      l_sumValue.x = l_sumValue.x + opacity_" + std::to_string(i) + " * scalar_" + std::to_string(i) + ".x;"
        );
      }
    }*/
    //else if (mapper->GetBlendMode() == vtkMultiVolumeMapper::COMPOSITE_BLEND)
    //{
      /*if (noOfComponents > 1 && independentComponents)
      {
        shaderStr += std::string("\
        ...
        if (glMapper->GetUseDepthPass() && glMapper->GetCurrentPass() ==
            vtkOpenGLGPUMultiVolumeRayCastMapper::DepthPass)
        {
          shaderStr += std::string("\
            ...
            maskType != vtkGPUMultiVolumeRayCastMapper::LabelMapMaskType)
        {
          shaderStr += std::string("\
          \n        // Data fetching from the red channel of volume texture\
               ...
               vtkOpenGLGPUMultiVolumeRayCastMapper::DepthPass)
      {
		  ...
		  shaderStr += "\n      srcColor.a = computeOpacity(i, scalar);";
      }
      else
      {
         if (!mask || !maskInput ||
             maskType != vtkGPUMultiVolumeRayCastMapper::LabelMapMaskType)
         {
			...
			shaderStr += "\n      if (srcColor.a > 0.) {";
			for (int i = 0; i < mapper->GetNumberOfInputConnections(0); ++i)
				shaderStr +="\n        srcColor += computeColor(i, scalar, srcColor.a);";
         }
         ...
         shaderStr += std::string("\
           ...
           \n        g_fragColor += (1.0f - g_fragColor.a) * srcColor;"
         );
  ...
  std::string PickingActorPassExit(vtkRenderer* vtkNotUsed(ren),
    vtkMultiVolumeMapper* vtkNotUsed(mapper), vtkMultiVolume2* vtkNotUsed(vol))
  {
    return std::string("\
                          ...
                          int independentComponents = 0)
  {
    ...
    if (glMapper->GetUseDepthPass() && glMapper->GetCurrentPass() ==
        vtkOpenGLGPUMultiVolumeRayCastMapper::DepthPass &&
        mapper->GetBlendMode() == vtkMultiVolumeMapper::COMPOSITE_BLEND)
    {
      return std::string();
    }
    else if (mapper->GetBlendMode() == vtkMultiVolumeMapper::MAXIMUM_INTENSITY_BLEND)
    {
      /*if (noOfComponents > 1 && independentComponents) // TODO
      {
        return std::string("\
          \n   g_srcColor = vec4(0);\
          \n   for (int i = 0; i < in_noOfComponents; ++i)\
      ...
      else
      {
		  ...
		  for (int i = 0; i < mapper->GetNumberOfInputConnections(0); ++i)
			  str += "\
         \n  srcColor = computeColor_" + std::to_string(i) + "(l_maxValue,\
         \n                            computeOpacity_" + std::to_string(i) + "(l_maxValue));";
                              ...
                              vtkMultiVolume2* vtkNotUsed(vol))
  {
    return std::string("\
      ...
      \n  // in_projectionMatrix is inversed because of way VT\
      \n  // From eye coordinates to texture coordinates\
      \n  terminatePoint = ip_inverseTextureDataAdjusted[i] *\
      \n                   in_inverseVolumeMatrix[i] *\
      \n                   in_inverseModelViewMatrix *\
      \n                   in_inverseProjectionMatrix *\
      \n                   terminatePoint;\
      \n  terminatePoint /= terminatePoint.w;\
      \n\
      \n  g_terminatePointMax[i] = length(terminatePoint.xyz - g_dataPos[i].xyz) /\
      \n                           length(g_dirStep[i]);\
      \n  g_currentT[i] = 0.0;\
      \n}"); // end for
  }
                                        ...
                                        vtkMultiVolume2* vtkNotUsed(vol))
  {
    return std::string("\
      ...
      \n        g_currentT[i] >= g_terminatePointMax[i])\
      \n    {\
      \n      break;\
      \n    }\
      \n    ++g_currentT[i];"
  ...
  std::string DepthPassImplementation(
    vtkRenderer* vtkNotUsed(ren), vtkMultiVolumeMapper* vtkNotUsed(mapper),
    vtkMultiVolume2* vtkNotUsed(vol))
  {
  return std::string("\
...
#endif // vtkMultiVolumeShaderComposer_h
// VTK-HeaderTest-Exclude: vtkMultiVolumeShaderComposer.h
