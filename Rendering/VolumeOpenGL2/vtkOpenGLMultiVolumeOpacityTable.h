/*=========================================================================
  ...
  Module:    vtkOpenGLMultiVolumeOpacityTable.h
...
#ifndef vtkOpenGLMultiVolumeOpacityTable_h
#define vtkOpenGLMultiVolumeOpacityTable_h
...
//----------------------------------------------------------------------------
class vtkOpenGLMultiVolumeOpacityTable : public vtkObject
{
public:

  static vtkOpenGLMultiVolumeOpacityTable* New();
              ...
              vtkOpenGLRenderWindow* renWin)
  {
    ...
    if(scalarOpacity->GetMTime() > this->BuildTime ||
       ...
       (blendMode == vtkMultiVolumeMapper::COMPOSITE_BLEND &&
       ...
       needUpdate || !this->TextureObject->GetHandle())
    {
      ...
      // TODO Fix this code for sample distance in three dimensions
        if(blendMode == vtkMultiVolumeMapper::COMPOSITE_BLEND)
        {
        ...
        else if (blendMode==vtkMultiVolumeMapper::ADDITIVE_BLEND)
        {
          float* ptr = this->Table;
          double factor = sampleDistance/unitDistance;
          int i = 0;
...
protected:
  ...
  //--------------------------------------------------------------------------
  vtkOpenGLMultiVolumeOpacityTable(int width = 1024)
  {
      this->TextureObject = NULL;
      this->LastBlendMode = vtkMultiVolumeMapper::MAXIMUM_INTENSITY_BLEND;
  ...
  //--------------------------------------------------------------------------
  ~vtkOpenGLMultiVolumeOpacityTable() VTK_OVERRIDE
  {
      if (this->TextureObject)
      {
        this->TextureObject->Delete();
...
private:
  vtkOpenGLMultiVolumeOpacityTable(const vtkOpenGLMultiVolumeOpacityTable&)
    VTK_DELETE_FUNCTION;
  vtkOpenGLMultiVolumeOpacityTable& operator=(const vtkOpenGLMultiVolumeOpacityTable&)
    VTK_DELETE_FUNCTION;
};

vtkStandardNewMacro(vtkOpenGLMultiVolumeOpacityTable);


////////////////////////////////////////////////////////////////////////////////
class vtkOpenGLMultiVolumeOpacityTables
{
public:
  //--------------------------------------------------------------------------
  vtkOpenGLMultiVolumeOpacityTables(unsigned int numberOfTables)
  {
    this->Tables.reserve(static_cast<size_t>(numberOfTables));

    for (unsigned int i = 0; i < numberOfTables; i++)
    {
      vtkOpenGLMultiVolumeOpacityTable* table = vtkOpenGLMultiVolumeOpacityTable::New();
      this->Tables.push_back(table);
    }
  }

  //--------------------------------------------------------------------------
  ~vtkOpenGLMultiVolumeOpacityTables()
  {
  ...
  //--------------------------------------------------------------------------
  vtkOpenGLMultiVolumeOpacityTable* GetTable(unsigned int i)
  {
...
private:
  std::vector<vtkOpenGLMultiVolumeOpacityTable*> Tables;

  vtkOpenGLMultiVolumeOpacityTables() VTK_DELETE_FUNCTION;

  vtkOpenGLMultiVolumeOpacityTables(const vtkOpenGLMultiVolumeOpacityTables &other) VTK_DELETE_FUNCTION;

  vtkOpenGLMultiVolumeOpacityTables &operator=(const vtkOpenGLMultiVolumeOpacityTables &other) VTK_DELETE_FUNCTION;
};

#endif // vtkOpenGLMultiVolumeOpacityTable_h
// VTK-HeaderTest-Exclude: vtkOpenGLMultiVolumeOpacityTable.h
