...
#ifndef vtkOpenGLGPUMultiVolumeRayCastMapper_h
#define vtkOpenGLGPUMultiVolumeRayCastMapper_h
...
class vtkMultiVolumeTexture;

//----------------------------------------------------------------------------
class VTKRENDERINGVOLUMEOPENGL2_EXPORT vtkOpenGLGPUMultiVolumeRayCastMapper :
...
public:
   ...
   */
  void SetPartitions(int i, unsigned short x, unsigned short y, unsigned short z);
   ...
   */
  bool PreLoadData(vtkRenderer* ren, vtkMultiVolume2* vol);

protected:
  vtkOpenGLGPUMultiVolumeRayCastMapper();
  ~vtkOpenGLGPUMultiVolumeRayCastMapper() VTK_OVERRIDE;
  ...
  void BuildDepthPassShader(vtkRenderer* ren, vtkMultiVolume2* vol,
                            int noOfComponents,
                            int independentComponents);

  // Description:
  // Build vertex and fragment shader for the volume rendering
  void BuildShader(vtkRenderer* ren, vtkMultiVolume2* vol, int noOfCmponents);
  ...
  void GPURender(vtkRenderer *ren, vtkMultiVolume2 *vol) VTK_OVERRIDE;
   ...
   */
  vtkMTimeType GetRenderPassStageMTime(vtkMultiVolume2* vol);
  ...
  void ReplaceShaderRenderPass(std::string& vertShader, std::string& fragShader,
                               vtkMultiVolume2* vol, bool prePass);

  /**
   *  Update parameters from RenderPass
   */
  void SetShaderParametersRenderPass(vtkMultiVolume2* vol);
  ...
  int    CurrentPass;
  vtkMultiVolumeTexture* GetVolumeTexture(int i);

private:
  class vtkInternal;
  vtkInternal* Impl;

  friend class vtkMultiVolumeTexture;
  std::map<int, vtkMultiVolumeTexture*> VolumeTexture;

  vtkImplicitFunction* NoiseGenerator;
  int NoiseTextureSize[2];

  vtkOpenGLGPUMultiVolumeRayCastMapper(const vtkOpenGLGPUMultiVolumeRayCastMapper&) VTK_DELETE_FUNCTION;
  void operator=(const vtkOpenGLGPUMultiVolumeRayCastMapper&) VTK_DELETE_FUNCTION;
};

#endif // vtkOpenGLGPUMultiVolumeRayCastMapper_h
