...
vtkMultiVolumeTexture::vtkMultiVolumeTexture()
...
//-----------------------------------------------------------------------------
vtkStandardNewMacro(vtkMultiVolumeTexture);

//-----------------------------------------------------------------------------
void vtkMultiVolumeTexture::SetMapper(vtkOpenGLGPUMultiVolumeRayCastMapper* mapper)
{
  if (mapper == NULL)
  {
...
//-----------------------------------------------------------------------------
bool vtkMultiVolumeTexture::LoadVolume(vtkRenderer* ren, vtkImageData* data,
  vtkDataArray* scalars, int const interpolation)
{
  ...
  {
    if (this->Mapper->CellFlag[index] == 1)
    {
...
//-----------------------------------------------------------------------------
void vtkMultiVolumeTexture::SetInterpolation(int const interpolation)
{
  this->InterpolationType = interpolation;
...
//-----------------------------------------------------------------------------
vtkMultiVolumeTexture::VolumeBlock* vtkMultiVolumeTexture::GetNextBlock()
{
...
//-----------------------------------------------------------------------------
void vtkMultiVolumeTexture::CreateBlocks(unsigned int const format,
  unsigned int const internalFormat, int const type)
{
  ...
  this->FullSizeAdjusted[0] = this->FullSize[0] - this->Mapper->CellFlag[index];
  this->FullSizeAdjusted[1] = this->FullSize[1] - this->Mapper->CellFlag[index];
  this->FullSizeAdjusted[2] = this->FullSize[2] - this->Mapper->CellFlag[index];
...
//-----------------------------------------------------------------------------
void vtkMultiVolumeTexture::AdjustExtentForCell(Size6& extent)
{
  int i = 1;
  while (i < 6)
  {
...
//-----------------------------------------------------------------------------
vtkMultiVolumeTexture::Size3 vtkMultiVolumeTexture::ComputeBlockSize(int* extent)
{
  int i = 0;
  Size3 texSize;
  while(i < 3)
...
//-----------------------------------------------------------------------------
bool vtkMultiVolumeTexture::LoadTexture(int const interpolation, VolumeBlock* volBlock)
{
...
//-----------------------------------------------------------------------------
void vtkMultiVolumeTexture::ReleaseGraphicsResources(vtkWindow* win)
{
  if (this->Texture)
  {
...
//-----------------------------------------------------------------------------
void vtkMultiVolumeTexture::ClearBlocks()
{
...
//-----------------------------------------------------------------------------
void vtkMultiVolumeTexture::SplitVolume(vtkImageData* imageData, Size3 const & part)
{
...
//-----------------------------------------------------------------------------
void vtkMultiVolumeTexture::SelectTextureFormat(unsigned int& format,
...
//----------------------------------------------------------------------------
void vtkMultiVolumeTexture::UpdateInterpolationType(int const interpolation)
{
...
//----------------------------------------------------------------------------
void vtkMultiVolumeTexture::SortBlocksBackToFront(vtkRenderer *ren,
...
//-----------------------------------------------------------------------------
void vtkMultiVolumeTexture::SetPartitions(int const x, int const y, int const z)
{
  if (x > 0 && y > 0 && z > 0)
  {
    if (x > 1 || y > 1 || z > 1)
...
//-----------------------------------------------------------------------------
void vtkMultiVolumeTexture::PrintSelf(ostream& os, vtkIndent indent)
{
...
//-----------------------------------------------------------------------------
bool vtkMultiVolumeTexture::AreDimensionsValid(vtkTextureObject* texture, int const width,
...
//-----------------------------------------------------------------------------
bool vtkMultiVolumeTexture::SafeLoadTexture(vtkTextureObject* texture, int const width,
