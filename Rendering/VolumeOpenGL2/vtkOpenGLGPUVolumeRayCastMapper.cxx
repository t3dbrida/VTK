...
//REMOVE//  void InitializeMaskTransfer(vtkRenderer* ren, VolumeInput& input);
  ...
  std::map<vtkVolume*, uint64_t> MaskUpdateTime;
  ...
  std::map<vtkImageData*, vtkSmartPointer<vtkTextureObject>> OctahedralGradientTextures;
...
{
    if (index < this->MaxGradientTextures)
    {
        ...
        auto& gradientVolumeTexture = this->OctahedralGradientTextures[volume];
...
bool vtkOpenGLGPUVolumeRayCastMapper::vtkInternal::LoadMasks(vtkRenderer* ren)
{
  ...
  for (const auto& mask : this->Parent->Masks)
  {
    ...
    if (maskInput && (this->MaskUpdateTime.find(volumePtr) == this->MaskUpdateTime.end() || maskInput->GetMTime() > this->MaskUpdateTime[volumePtr]))
    {
      ...
      this->MaskUpdateTime[volumePtr] = maskInput->GetMTime();
    }
    else if (!maskInput)
    {
...
bool vtkOpenGLGPUVolumeRayCastMapper::vtkInternal::LoadRegions(vtkRenderer* ren)
{
  ...
  {
    const struct vtkVolumeProperty::BitRegion& bitRegion = in.second.Volume->GetProperty()->GetBitRegion();
    vtkImageData* const bitRegionMask = bitRegion.mask;
    auto it = this->RegionMaskTextures.find(in.first);
    if (it != this->RegionMaskTextures.end())
    {
      RegionMaskTexture& regionTexture = it->second;
      if (bitRegionMask != regionTexture.image)
      {
        regionTexture.texture->ReleaseGraphicsResources(ren->GetRenderWindow());
        this->RegionMaskTextures.erase(it);
        it = this->RegionMaskTextures.end();
      }
    }
    ...
    if (bitRegionMask)
    {
      ...
      {
        update = regionTexture.timestamp < bitRegionMask->GetMTime();
      }

      if (update && context)
      {
        int isCellData;
        vtkDataArray* arr = this->Parent->GetScalars(bitRegionMask, this->Parent->ScalarMode, this->Parent->ArrayAccessMode, this->Parent->ArrayId, this->Parent->ArrayName, isCellData);
...
void vtkOpenGLGPUVolumeRayCastMapper::vtkInternal::SetupRegionDepthFramebuffer(vtkRenderer* ren, bool hasRegions)
{
  ...
  if (hasRegions && context)
  {
      ...
      {
          if (this->RegionDepthTextureObject->GetWidth() != this->WindowSize[0] || this->RegionDepthTextureObject->GetHeight() != this->WindowSize[1])
          {
...
*/
      ...
      double outSpacing[3] = {
        double(renderWindowSize[0]) / this->WindowSize[0],
        double(renderWindowSize[1]) / this->WindowSize[1],
        1.
      };
  ...
  vtkRenderer* ren, vtkShaderProgram* prog, vtkVolume* vol, int numberOfSamplers)
{
  ...
  if (this->MultiVolume)
  {
      for (const int port : this->Parent->Ports)
      {
          ...
          value = volumeProperty->GetOpacity(0);
          opacity.emplace_back(value);
      }
  }
  else
  {
  ...
  vtkRenderer* ren, vtkVolume* vol, double loadedBounds[6])
{
  ...
  int const result = vtkMath::PlaneIntersectsAABB(loadedBounds, camPlaneNormal, camNearPoint);
...
//----------------------------------------------------------------------------
bool vtkOpenGLGPUVolumeRayCastMapper::vtkInternal::IsGeometryUpdateRequired(vtkRenderer* ren, vtkVolume* vol, double loadedBounds[6])
{
  ...
  vtkWindow* window)
{
  ...
  for (auto& currentMask : this->Impl->CurrentMasks)
  {
    currentMask.second->ReleaseGraphicsResources(window);
  }
  for (auto& rmt : this->Impl->RegionMaskTextures)
  {
      rmt.second.texture->ReleaseGraphicsResources(window);
  }
                                                           ...
                                                           int numComps)
{
  ...
  if (this->Impl->MultiVolume)
  {
    ...
    vtkShaderProgram::Substitute(fragmentShader,
      "//VTK::Shading::Impl",
      vtkvolume::ShadingMultipleInputs(this, this->AssembledInputs, masks, this->Impl->CurrentMasks));
  }
  else
  {
  ...
  int numComps)
{
  ...
  vtkShaderProgram::Substitute(fragmentShader,
    ...
    vtkvolume::ComputeGradientDeclaration(this, this->AssembledInputs, this->Impl->MaxGradientTextures));
  ...
  vtkMatrix4x4* modelViewMat)
{
  ...
  for (auto& input : this->Parent->AssembledInputs)
  {
    ...
    if (gradTexIt != this->OctahedralGradientTextures.end())
    {
        ...
        const auto& gradientVolumeTexture = gradTexIt->second;
        if (gradientVolumeTexture)
        {
  ...
  vtkShaderProgram* prog, vtkRenderer *ren, int independent, int numComp)
{
  ...
  // setup alpha compensation when downsampling
  if (this->Parent->AutoAdjustSampleDistances == true)
  {
      prog->SetUniformf("in_downsampleCompensation", 1.f);
  }
  else
  {
...
void vtkOpenGLGPUVolumeRayCastMapper::vtkInternal::SetRegionShaderParameters(vtkShaderProgram* prog)
{
  ...
  for (const auto& in : this->Parent->AssembledInputs)
  {
    ...
    if (region.mask && !region.colors.empty())
    {
      ...
      if (it != this->RegionMaskTextures.end())
      {
        vtkTextureObject* const to = it->second.texture;
        to->Activate();
        const std::string iStr = std::to_string(regionIndex);
        prog->SetUniformi(("in_regionMask[" + iStr + ']').c_str(), to->GetTextureUnit());
        ++regionIndex;

        for (std::size_t i = 0; i < region.colors.size(); ++i)
        {
          prog->SetUniform4f(("in_regionColor[" + std::to_string(regionOffset) + "]").c_str(), region.colors[i].GetData());
          ++regionOffset;
        }
      }
    }
  ...
  const int numComp)
{
  ...
  for (const auto& rmt : this->RegionMaskTextures)
  {
    rmt.second.texture->Deactivate();
  }
                                                                     ...
                                                                     vtkShaderProgram* prog)
{
  ...
  //  for (const auto& rmt : this->RegionMaskTextures)
  //  {
  //    rmt.second.texture->SortBlocksBackToFront(ren, vol->GetMatrix());
  //  }
  //}
