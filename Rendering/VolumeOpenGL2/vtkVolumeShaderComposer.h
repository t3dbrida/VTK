...
namespace vtkvolume
{
                                      ...
                                      int independentComponents)
  {
    ...
    toShaderStr << "float g_terminateT;\n";
    ...
    toShaderStr <<
        ...
        "\n"
        "    // Map from [0,63] → [-1,1]\n"
        "    vec2 e = vec2(float(ox), float(oy)) / 63.0 * 2.0 - 1.0;\n"
        "\n"
        "    // Reconstruct z component and fold lower hemisphere\n"
        ...
        "    vec3 n = normalize(v);\n"
        "\n"
        "    // Decode magnitude (0..1) and scale by max gradient\n"
                       ...
                       int lightingComplexity)
  {
    ...
    if (inputCount > 1)
    {
        shaderStr += "\
          ...
          \n    g_tDelta[i] = vec3(g_rayDir.x != 0. ? (g_rayDirSign.x * cs.x / g_rayDir.x) : FLOAT_INF,\
    ...
    {
      shaderStr += "\
        \n  vec3 minRayJitter = vec3(FLOAT_INF);\
        \n  for (int i = 0; i < " + std::to_string(inputCount) + "; ++i)\
        \n  {\
      ...
      //if (shadeReqd)
      {
        if (lightingComplexity == 1)
        {
          ...
          shaderStr +=
            ...
            "\n  //finalColor.xyz = in_ambient[index] * color.rgb + attenuation * (diffuse + specular);"
                          ...
                          vtkVolume* vol)
  {
    ...
    if (numInputs > 1)
    {
        str +=
          ...
          "    if (t[i] != 0. && length(g_dirSteps[i]) < FLOAT_EPS)\n"
          "    {\n"
          "      t[i] = FLOAT_INF;\n"
          "    }\n"
          "  }\n"
    ...
    else
    {
        str += "\n"
            ...
            "\n"
            "  bool doingVol = false;\n"
            "  if (volT != FLOAT_INF && volT < segT) doingVol = true;\n"
    ...
    //        \n  l_normValues_" + ii + "[0] = FLOAT_INF_NEG; //-infinity\
    //        \n  l_normValues_" + ii + "[NUMBER_OF_CONTOURS_" + ii + " + 1] = FLOAT_INF; //+infinity\
