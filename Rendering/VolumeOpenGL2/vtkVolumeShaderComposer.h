                          ...
                          vtkVolume* vol)
  {
    ...
    else
    {
        str += "\n"
            ...
            "    if (volumeParameters.data[i].volumeVisibility.x == 1 || volumeParameters.data[i].noOfComponents_maskIndex_regionIndex_transfer2dIndex.z >= 0)\n"
            "    {\n"
            "      vec3 csHalf = /*.5 * */volumeParameters.data[i].cellSpacing.xyz; // halving removed to account for edges\n"
            ...
            if (bitRegion.mask)
            {
                str +=
                    ...
                    "      {\n"
                    "        vec3 cs = volumeParameters.data[0].cellSpacing.xyz;\n"
                    "        vec3 gridCorner = cs * floor((hitPoint + .5 * cs) / cs) - .5 * cs; // we move the enlarged volume by half voxel to properly work with flooring, then restore the original grid corner position\n"
                    "        vec3 nextGridLine = gridCorner + vec3(greaterThanEqual(g_rayDirSign, vec3(0.))) * cs;\n"
                    "        vec3 samplePos = gridCorner + cs; // idk why this works tbh, probably to account for sampling at voxel center is to add full voxel (we must go back to (0,0,0) because tex. coords start there)\n"
                    "\n"
                    "        segDataPos = (ip_inverseTextureDataAdjusted * vec4(samplePos, 1.)).xyz;\n" // TODO these texture matrices might differ for regions
                    "\n"
                    ...
                    "        segNormal = -g_rayDirSign * vec3(lessThanEqual(segTMax, vec3(min(segTMax.x, min(segTMax.y, segTMax.z)))));\n"
                    "        segT = intersections[i].t;\n"
                    "        ++stepCounter;\n"
                    "      }\n"
                    "\n"
                  ;
