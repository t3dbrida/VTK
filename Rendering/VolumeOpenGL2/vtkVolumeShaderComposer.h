...
namespace vtkvolume
{
  ...
  {
    std::string shaderStr;
    auto* glMapper = static_cast<vtkOpenGLGPUVolumeRayCastMapper*>(mapper);
    //int i = 0;
    //for (auto& input : inputs)
    {
      ...
      shaderStr +=
        "\n"
        "vec4 computeLighting(int index, vec4 color, vec4 gradient, int type)\n"
        "{\n"
      ...
      //if (shadeReqd)
      {
        ...
        else if (lightingComplexity == 3)
        {
          shaderStr +=
            ...
            "\n  vec3 normal = normalize(gradient.xyz);//normalize((in_textureToEye * vec4(gradient.xyz, 0.0)).xyz);"
            "\n  vec3 lightDir;"
            "\n  for (int lightNum = 0; lightNum < in_numberOfLights; lightNum++)"
            "\n  {"
            "\n    float attenuation = 1.0;"
            "\n    // directional"
            "\n    lightDir = (in_inverseVolumeMatrix * vec4(in_lightDirection[lightNum], 0.)).xyz;"
            ...
            "\n    else"
            "\n    {"
            "\n      vertLightDirection = ((in_inverseVolumeMatrix * vec4(in_lightPosition[lightNum], 1.)).xyz - g_fragWorldPos.xyz);"
            "\n      float distance = length(vertLightDirection);"
            "\n      vertLightDirection = normalize(vertLightDirection);"
            "\n      attenuation = 1.0 / (in_lightAttenuation[lightNum].x + in_lightAttenuation[lightNum].y * distance + in_lightAttenuation[lightNum].z * distance * distance);"
            ...
            "\n      diffuse += (df * in_lightDiffuseColor[lightNum]) * color.rgb;"
            "\n    }"
            ...
            "\n      specular += (sf * in_lightSpecularColor[lightNum]) * color.rgb;"
            "\n    }"
            "\n    ambient += in_lightAmbientColor[lightNum];"
            "\n  }"
            "\n  float shadingFactor = smoothstep(in_shadingGradientScales[index].s, in_shadingGradientScales[index].t, gradient.w);"
            "\n  finalColor.xyz = mix(color.rgb, in_ambient[index] * ambient * color.rgb, shadingFactor); // apply color for sf=0, ambient for sf=1"
            "\n  finalColor.xyz += shadingFactor * (in_diffuse[index] * diffuse + in_specular[index] * specular);"
          ;
        }
      }
      //else
    ...
    vtkOpenGLGPUVolumeRayCastMapper::VolumeInputMap& inputs)
  {
    std::ostringstream ss;
    {
      ss <<
        ...
        "{\n"
        "  return computeLighting(index, color, gradient, TYPE_VOLUME);\n"
        "}\n"
        "\n";
    }
                                      ...
                                      int independentComponents)
  {
      //if (noOfComponents == 1)
      {
        ...
        return
          ...
          "  return computeLighting(0, color, g_gradients[0], TYPE_VOLUME);\n"
          "}\n";
      }
      //else if (noOfComponents > 1 && independentComponents)
      //{
  ...
  std::string ShadingInit(vtkRenderer* vtkNotUsed(ren),
                          ...
                          vtkVolume* vol)
  {
    auto gpuMapper = vtkOpenGLGPUVolumeRayCastMapper::SafeDownCast(mapper);
    const int numInputs = gpuMapper->GetInputCount();
    ...
    if (numInputs > 1)
    {
        str += "  Interval intervals[" + numInputsStr + "];\n";
        for (int i = 0; i < numInputs; ++i)
        {
            std::string iStr = std::to_string(i);
            str +=
                "  vec3 dataPos" + iStr + " = vec3(FLOAT_MAX, FLOAT_MAX, FLOAT_MAX);\n"
                "  float t" + iStr + " = FLOAT_MAX;\n"
                "\n"
                "  {\n"
                "    intervals[" + iStr + "].tEnter = FLOAT_MAX;\n"
                "    intervals[" + iStr + "].tExit = -FLOAT_MAX;\n"
                "    intervals[" + iStr + "].valid = false;\n"
                "    if (volumeParameters.data[" + iStr + "].volumeVisibility.x == 1 || volumeParameters.data[" + iStr + "].noOfComponents_maskIndex_regionIndex_transfer2dIndex.z >= 0)\n"
                "    {\n"
                "      mat4 globalToLocalDatasetTransform = volumeParameters.data[" + iStr + "].inverseVolumeMatrix * in_volumeMatrix;\n"
                ...
                "      vec2 interval = intersectRayBox(localEye, localDir, volumeParameters.data[" + iStr + "].boundsMin.xyz, volumeParameters.data[" + iStr + "].boundsMax.xyz);\n"
                "      intervals[" + iStr + "].valid = interval.x < interval.y && interval.x < FLOAT_MAX && interval.y > 0.;\n"
                "      if (intervals[" + iStr + "].valid == true)\n"
                "      {\n"
                "        mat4 localToGlobalDatasetTransform = in_inverseVolumeMatrix * volumeParameters.data[" + iStr + "].volumeMatrix;\n"
                ...
                "        intervals[" + iStr + "].tEnter = dot(posEnter - g_eyePosObj.xyz, g_rayDir) / g_rayDirDot;\n"
                "        intervals[" + iStr + "].tExit = dot(posExit - g_eyePosObj.xyz, g_rayDir) / g_rayDirDot;\n"
                "        dataPos" + iStr + " = (in_inverseTextureDatasetMatrix * vec4(g_eyePosObj.xyz + intervals[" + iStr + "].tEnter * g_rayDir, 1.)).xyz + g_rayJitter[" + iStr + " + 1];\n"
                "        t" + iStr + " = intervals[" + iStr + "].tEnter;\n"
                "      }\n"
                "    }\n"
                "  }\n"
                "\n"
    ...
    else
    {
        str += "\n"
            ...
            "  float volT = FLOAT_MAX, segT = FLOAT_MAX;\n"
            "  vec3 segTMax;\n"
            "  vec4 segNormal;\n"
            ...
            str +=
                "      if (volumeParameters.data[i].volumeVisibility.x == 1 || (" + std::string(gpuMapper->GetSimpleRegionRendering() ? "true" : "false") + " && volumeParameters.data[i].noOfComponents_maskIndex_regionIndex_transfer2dIndex.z >= 0))\n"
                "      {\n"
  ...
  {
    std::ostringstream toShaderStr;
    toShaderStr << "    g_exit = true;\n";
    for (int i = 0; i < inputs.size(); ++i)
    {
        std::string iStr = std::to_string(i);
        toShaderStr <<
          "\n"
          "    g_dataPos = dataPos" + iStr + ";\n"
          "    texPos = (volumeParameters.data[" + iStr + "].cellToPoint * volumeParameters.data[" + iStr + "].inverseTextureDatasetMatrix * volumeParameters.data[" + iStr + "].inverseVolumeMatrix *\n"
          "             in_volumeMatrix * in_textureDatasetMatrix * vec4(g_dataPos.xyz, 1.0)).xyz;\n"
          "    if (t" + iStr + " != FLOAT_MAX && g_terminatePosLength >= length((g_eyePosObj.xyz + t" + iStr + " * g_rayDir.xyz) - g_eyePosObj.xyz) && all(lessThanEqual(texPos, volumeParameters.data[" + iStr + "].texMax.xyz)) && all(greaterThanEqual(texPos, volumeParameters.data[" + iStr + "].texMin.xyz)))\n"
          "    {\n"
        ;
          ...
          default:
          {
            ...
            //for (auto& item : inputs)
            {
              ...
              toShaderStr <<
                  ...
                  "\n"
                  "        if (volumeParameters.data[" + iStr + "].noOfComponents_maskIndex_regionIndex_transfer2dIndex.y != -1)\n"
                  "        {\n"
                  "          noMask = false;\n"
                  "          if (sampleMask(volumeParameters.data[" + iStr + "].noOfComponents_maskIndex_regionIndex_transfer2dIndex.y, texPos).r > 0)\n"
                  "          {\n"
                  "            maskedByRegion = true;\n"
                  "          }\n"
                  "        }\n"
                  "\n"
                  "        vec3 p = vec3(volumeParameters.data[" + iStr + "].inverseVolumeMatrix * in_volumeMatrix * in_textureDatasetMatrix * vec4(g_dataPos.xyz, 1.)); \n"
                  "        if (volumeParameters.data[" + iStr + "].boxMaskAxisX.xyz != vec3(0.) && volumeParameters.data[" + iStr + "].boxMaskAxisY.xyz != vec3(0.) && volumeParameters.data[" + iStr + "].boxMaskAxisZ.xyz != vec3(0.))\n"
                  "        {\n"
                  "          noMask = false;\n"
                  "          float projX = dot(p - volumeParameters.data[" + iStr + "].boxMaskOrigin.xyz, volumeParameters.data[" + iStr + "].boxMaskAxisX.xyz) / dot(volumeParameters.data[" + iStr + "].boxMaskAxisX.xyz, volumeParameters.data[" + iStr + "].boxMaskAxisX.xyz);\n"
                  "          float projY = dot(p - volumeParameters.data[" + iStr + "].boxMaskOrigin.xyz, volumeParameters.data[" + iStr + "].boxMaskAxisY.xyz) / dot(volumeParameters.data[" + iStr + "].boxMaskAxisY.xyz, volumeParameters.data[" + iStr + "].boxMaskAxisY.xyz);\n"
                  "          float projZ = dot(p - volumeParameters.data[" + iStr + "].boxMaskOrigin.xyz, volumeParameters.data[" + iStr + "].boxMaskAxisZ.xyz) / dot(volumeParameters.data[" + iStr + "].boxMaskAxisZ.xyz, volumeParameters.data[" + iStr + "].boxMaskAxisZ.xyz);\n"
                  "          maskedByBox = projX >= 0. && projX <= 1. &&\n"
                  "                        projY >= 0. && projY <= 1. &&\n"
                  "                        projZ >= 0. && projZ <= 1.;\n"
                  "        }\n"
                  "        {\n"
                  "          float cylinderMaskRadius = volumeParameters.data[" + iStr + "].cylinderMaskAxis_cylinderMaskRadius.w;\n"
                  "          if (cylinderMaskRadius > 0.)\n"
                  "          {\n"
                  "            noMask = false;\n"
                  "            vec3 cylinderMaskCenter = volumeParameters.data[" + iStr + "].cylinderMaskCenter.xyz;\n"
                  "            vec3 cylinderMaskAxis = volumeParameters.data[" + iStr + "].cylinderMaskAxis_cylinderMaskRadius.xyz;\n"
                  ...
                  "          }\n"
                  "        }\n"
                  "        if (g_skip == false && (noMask || maskedByBox || maskedByCylinder || maskedByRegion))\n"
                  "        {\n"
                  "          int colorCount = 0;\n"
                  "          vec4 scalar = sampleVolume(" + iStr + ", texPos);\n"
                  "          scalar = volumeParameters.data[" + iStr + "].volumeScale * scalar + volumeParameters.data[" + iStr + "].volumeBias;\n"
                  "\n"
                  "          vec4 regionResultColor = vec4(0.);\n"
                  "          if (volumeParameters.data[" + iStr + "].noOfComponents_maskIndex_regionIndex_transfer2dIndex.z >= 0)\n"
                  "          {\n"
                  "            uvec4 regionMaskValue = sampleRegionMask(volumeParameters.data[" + iStr + "].noOfComponents_maskIndex_regionIndex_transfer2dIndex.z, texPos);\n"
                  "            int regionCount = in_regionOffset[" + iStr + " + 1] - in_regionOffset[" + iStr + "];\n"
                  ...
                  "              {\n"
                  "                vec4 regionColor = getRegionColor(in_regionOffset[" + iStr + "] + regionIndex);\n"
                  "                if (regionColor.a > 0.)\n"
                  "                {\n"
                  ...
                  "\n"
                  "          g_srcColor = vec4(0.);\n"
                  "          if (volumeParameters.data[" + iStr + "].volumeVisibility.x == 1)\n"
                  "          {\n"
                  "            vec4 color = vec4(0.);\n"
                  "            vec4 grad = vec4(0.);\n"
                  "\n"
                  "            if (volumeParameters.data[" + iStr + "].noOfComponents_maskIndex_regionIndex_transfer2dIndex.x == 1)\n"
                  "            {\n"
                  "              scalar = vec4(scalar.r);\n"
                  "              g_gradients[" + iStr + "] = computeGradient(" + iStr + ", texPos);\n"
                  "              color = sampleTransfer2D(" + iStr + ", vec2(scalar.r, g_gradients[" + iStr + "].w));\n"
                  "              grad = g_gradients[" + iStr + "];\n"
                  "            }\n"
                  "            else\n"
                  "            {\n"
                  "              color = vec4(scalar.rgb, computeOpacity(" + iStr + ", scalar));\n"
                  "              grad = computeGradient(" + iStr + ", texPos);\n"
                  "            }\n"
                  "            g_srcColor = computeColor(" + iStr + ", color, grad);\n"
                  "\n"
                  "            if (g_srcColor.a > 0.)\n"
                  "            {\n"
                  "              ++colorCount;\n"
                  ...
                  "          {\n"
                  "            ++colorCount;\n"
                  "            regionResultColor = computeLighting(" + iStr + ", regionResultColor, computeGradient(" + iStr + ", texPos), TYPE_REGION);\n"
                  ...
                  "    if (t"+ iStr + " != FLOAT_MAX)\n"
                  "    {\n"
                  "      dataPos" + iStr + " = dataPos" + iStr + " + g_dirSteps[" + iStr + "];\n"
                  "      t" + iStr + " = dot((in_textureDatasetMatrix * vec4(dataPos" + iStr + ", 1.) - g_eyePosObj).xyz, g_rayDir) / g_rayDirDot;\n"
                  "      if (t" + iStr + " < intervals[" + iStr + "].tExit + FLOAT_EPS)\n"
                  "      {\n"
                  "        g_exit = false;\n"
                  "      }\n"
                  "    }\n"
      ...
      else
      {
        ...
        if (bitRegion.mask)
        {
            if (!glMapper->GetSimpleRegionRendering())
            {
                shaderStr +=
                    "\n"
                    "      else if (g_dataPos.x >= 0. && g_dataPos.x <= 1. &&\n"
        ...
        // TODO!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! don't do downsample compensation for minecraft regions
        if (glMapper->GetSimpleRegionRendering())
        {
          shaderStr += "\
           \n        if (colorCount != 0)\
