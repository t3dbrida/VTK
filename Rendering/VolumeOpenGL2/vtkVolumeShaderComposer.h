...
namespace vtkvolume
{
                                      ...
                                      int independentComponents)
  {
    ...
    toShaderStr <<
      "\n"
      "vec3 sampleGradient(int index, vec3 uvw)\n"
      "{\n"
      "  uint x = sampleGradientVolume(index, uvw).r;\n"
      "  float u = float(bitfieldExtract(x, 8, 8)) / 255.0 * 2.0 - 1.0;\n"
      "  float v = float(bitfieldExtract(x, 0, 8)) / 255.0 * 2.0 - 1.0;\n"
                          ...
                          vtkVolume* vol)
  {
    ...
    if (numInputs > 1)
    {
        str +=
          ...
          "  {\n"
          "    dataPos[i] = vec3(FLOAT_MAX, FLOAT_MAX, FLOAT_MAX);\n"
          "    t[i] = FLOAT_MAX;\n"
          "\n"
          ...
          "        dataPos[i] = (in_inverseTextureDatasetMatrix * vec4(g_eyePosObj.xyz + intervals[i].tEnter * g_rayDir, 1.)).xyz + g_rayJitter[i + 1];\n"
          "        t[i] = intervals[i].tEnter;\n"
          "      }\n"
          "    }\n"
          "\n"
          "    // dirSteps can't be too small\n"
          "    if (length(g_dirSteps[i]) < 1e-6)\n"
          "    {\n"
          "      t[i] = FLOAT_MAX;\n"
          "    }\n"
          "  }\n"
                                    ...
                                    const std::map<vtkVolume*, vtkSmartPointer<vtkVolumeTexture>>& masks)
  {
    ...
    toShaderStr <<
      ...
      "    if (minT != FLOAT_MAX && g_terminatePosLength >= length((g_eyePosObj.xyz + minT * g_rayDir.xyz) - g_eyePosObj.xyz) && all(lessThanEqual(texPos, volumeParameters.data[volumeIndex].texMax.xyz)) && all(greaterThanEqual(texPos, volumeParameters.data[volumeIndex].texMin.xyz)))\n"
      "    {\n"
    ;
      ...
      default:
      {
        ...
        //for (auto& item : inputs)
        {
          ...
          toShaderStr <<
              ...
              "\n"
              "        if (volumeParameters.data[volumeIndex].noOfComponents_maskIndex_regionIndex_transfer2dIndex.y != -1)\n"
              "        {\n"
              "          noMask = false;\n"
              "          if (sampleMask(volumeParameters.data[volumeIndex].noOfComponents_maskIndex_regionIndex_transfer2dIndex.y, texPos).r > 0)\n"
              "          {\n"
              "            maskedByRegion = true;\n"
              "          }\n"
              "        }\n"
              "\n"
              "        vec3 p = vec3(volumeParameters.data[volumeIndex].inverseVolumeMatrix * in_volumeMatrix * in_textureDatasetMatrix * vec4(g_dataPos.xyz, 1.)); \n"
              "        if (volumeParameters.data[volumeIndex].boxMaskAxisX.xyz != vec3(0.) && volumeParameters.data[volumeIndex].boxMaskAxisY.xyz != vec3(0.) && volumeParameters.data[volumeIndex].boxMaskAxisZ.xyz != vec3(0.))\n"
              "        {\n"
              "          noMask = false;\n"
              "          float projX = dot(p - volumeParameters.data[volumeIndex].boxMaskOrigin.xyz, volumeParameters.data[volumeIndex].boxMaskAxisX.xyz) / dot(volumeParameters.data[volumeIndex].boxMaskAxisX.xyz, volumeParameters.data[volumeIndex].boxMaskAxisX.xyz);\n"
              "          float projY = dot(p - volumeParameters.data[volumeIndex].boxMaskOrigin.xyz, volumeParameters.data[volumeIndex].boxMaskAxisY.xyz) / dot(volumeParameters.data[volumeIndex].boxMaskAxisY.xyz, volumeParameters.data[volumeIndex].boxMaskAxisY.xyz);\n"
              "          float projZ = dot(p - volumeParameters.data[volumeIndex].boxMaskOrigin.xyz, volumeParameters.data[volumeIndex].boxMaskAxisZ.xyz) / dot(volumeParameters.data[volumeIndex].boxMaskAxisZ.xyz, volumeParameters.data[volumeIndex].boxMaskAxisZ.xyz);\n"
              "          maskedByBox = projX >= 0. && projX <= 1. &&\n"
              "                        projY >= 0. && projY <= 1. &&\n"
              "                        projZ >= 0. && projZ <= 1.;\n"
              "        }\n"
              "        {\n"
              "          float cylinderMaskRadius = volumeParameters.data[volumeIndex].cylinderMaskAxis_cylinderMaskRadius.w;\n"
              "          if (cylinderMaskRadius > 0.)\n"
              "          {\n"
              "            noMask = false;\n"
              "            vec3 cylinderMaskCenter = volumeParameters.data[volumeIndex].cylinderMaskCenter.xyz;\n"
              "            vec3 cylinderMaskAxis = volumeParameters.data[volumeIndex].cylinderMaskAxis_cylinderMaskRadius.xyz;\n"
              ...
              "        {\n"
              "          int colorCount = 0;\n"
              "          vec4 scalar = sampleVolume(volumeIndex, texPos);\n"
              "          scalar = volumeParameters.data[volumeIndex].volumeScale * scalar + volumeParameters.data[volumeIndex].volumeBias;\n"
              "\n"
              "          vec4 regionResultColor = vec4(0.);\n"
              "          if (volumeParameters.data[volumeIndex].noOfComponents_maskIndex_regionIndex_transfer2dIndex.z >= 0)\n"
              "          {\n"
              "            uvec4 regionMaskValue = sampleRegionMask(volumeParameters.data[volumeIndex].noOfComponents_maskIndex_regionIndex_transfer2dIndex.z, texPos);\n"
              "            int regionCount = in_regionOffset[volumeIndex + 1] - in_regionOffset[volumeIndex];\n"
              ...
              "              {\n"
              "                vec4 regionColor = getRegionColor(in_regionOffset[volumeIndex] + regionIndex);\n"
              "                if (regionColor.a > 0.)\n"
              "                {\n"
              ...
              "            vec4 grad = vec4(0.);\n"
              "\n"
              "            if (volumeParameters.data[volumeIndex].noOfComponents_maskIndex_regionIndex_transfer2dIndex.x == 1)\n"
              "            {\n"
              "              scalar = vec4(scalar.r);\n"
              "              g_gradients[volumeIndex] = computeGradient(volumeIndex, texPos);\n"
              "              color = sampleTransfer2D(volumeIndex, vec2(scalar.r, g_gradients[volumeIndex].w));\n"
              "              grad = g_gradients[volumeIndex];\n"
              "            }\n"
              "            else\n"
              "            {\n"
              "              color = vec4(scalar.rgb, computeOpacity(volumeIndex, scalar));\n"
              "              grad = computeGradient(volumeIndex, texPos);\n"
              "            }\n"
              "            g_srcColor = computeColor(volumeIndex, color, grad);\n"
              "\n"
              "            if (g_srcColor.a > 0.)\n"
              "            {\n"
              "              ++colorCount;\n"
              ...
              "    if (minT != FLOAT_MAX)\n"
              "    {\n"
              "      dataPos[volumeIndex] = dataPos[volumeIndex] + g_dirSteps[volumeIndex];\n"
              "      t[volumeIndex] = dot((in_textureDatasetMatrix * vec4(dataPos[volumeIndex], 1.) - g_eyePosObj).xyz, g_rayDir) / g_rayDirDot;\n"
              "      if (isnan(t[volumeIndex]) || t[volumeIndex] > intervals[volumeIndex].tExit + FLOAT_EPS)\n"
              "      {\n"
              "          t[volumeIndex] = FLOAT_MAX;\n"
              "      }\n"
              "    }\n"
              "\n"
              "    g_exit = true;\n"
