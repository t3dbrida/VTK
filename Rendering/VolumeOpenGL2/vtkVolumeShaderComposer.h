                          ...
                          vtkVolume* vol)
  {
    ...
    else
    {
        str += "\n"
            ...
            "      vec2 interval = intersectRayBox(g_eyePosObj.xyz + .5 * cs, g_rayDir, bboxMin + .5 * cs, bboxMax + .5 * cs);\n"
            "      intervals[i].valid = interval.x < interval.y && interval.x < FLOAT_MAX && interval.y > 0.;\n"
            "      if (intervals[i].valid == true)\n"
            "      {\n"
            ...
            if (bitRegion.mask)
            {
                str +=
                    ...
                    "\n"
                    "        segDataPos = (in_inverseTextureDatasetMatrix * vec4(gridCorner, 1.)).xyz;\n" // TODO these texture matrices might differ for regions
                    "        segTMax = vec3(g_rayDir.x != 0. ? (g_rayDirSign.x * (nextGridLine.x - g_eyePosObj.x) / abs(g_rayDir.x)) : FLOAT_MAX,\n"
                    "                       g_rayDir.y != 0. ? (g_rayDirSign.y * (nextGridLine.y - g_eyePosObj.y) / abs(g_rayDir.y)) : FLOAT_MAX,\n"
                    "                       g_rayDir.z != 0. ? (g_rayDirSign.z * (nextGridLine.z - g_eyePosObj.z) / abs(g_rayDir.z)) : FLOAT_MAX);\n"
                    "        vec3 segTMin = vec3(g_rayDir.x != 0. ? (g_rayDirSign.x * (gridCorner.x - g_eyePosObj.x) / abs(g_rayDir.x)) : FLOAT_EPS,\n"
                    "                            g_rayDir.y != 0. ? (g_rayDirSign.y * (gridCorner.y - g_eyePosObj.y) / abs(g_rayDir.y)) : FLOAT_EPS,\n"
                    "                            g_rayDir.z != 0. ? (g_rayDirSign.z * (gridCorner.z - g_eyePosObj.z) / abs(g_rayDir.z)) : FLOAT_EPS);\n"
