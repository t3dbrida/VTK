...
namespace vtkvolume
{
                                      ...
                                      int independentComponents)
  {
    ...
    toShaderStr <<
        ...
        "    // index is odd -> use lower 16 bits\n"
        "    if ((coord1d & 1u) == 0u)\n"
        "    {\n"
        "        word = word & 0xFFFFu;\n"
        "    }\n"
        "    else // index is even -> use upper 16 bits\n"
        "    {\n"
        "        word = word >> 16;\n"
        "    }\n"
        "    result = uvec4(word);\n"
    ...
    toShaderStr <<
        ...
        "\n"
        "    // Extract fields\n"
        "    uint u6 = (octVal >> 10) & 0x3Fu;\n"
        "    uint v6 = (octVal >> 4)  & 0x3Fu;\n"
        "    uint m4 =  octVal        & 0xFu;\n"
        "\n"
        "    // Convert back to [-1, 1]\n"
        "    float u = float(u6) / 63.0 * 2.0 - 1.0;\n"
        "    float v = float(v6) / 63.0 * 2.0 - 1.0;\n"
        "\n"
        "    // Octahedral reconstruction\n"
        "    float z = 1.0 - abs(u) - abs(v);\n"
        "    vec3 n = vec3(u, v, z);\n"
        "\n"
        "    // Fold back lower hemisphere\n"
        "    if (n.z < 0.0)\n"
        "    {\n"
        "        float ox = u;\n"
        "        float oy = v;\n"
