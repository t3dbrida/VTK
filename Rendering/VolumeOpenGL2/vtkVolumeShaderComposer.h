...
namespace vtkvolume
{
                                      ...
                                      int independentComponents)
  {
    ...
    toShaderStr << "struct SamplePointSet\n"
                   ...
                   "\n"
                   "  int currentIndex; // points to front of the set, can be -1 to indicate that no sample points are valid\n"
                   "};\n"
                   "\n";
    toShaderStr << "void updateMinSamplePoint(inout SamplePointSet samplePointSet)\n"
                   "{\n"
                   "  float minT = FLOAT_MAX;\n"
                   "  samplePointSet.currentIndex = -1;\n"
                   "  for (int i = 0; i < " << numInputs2x << "; ++i)\n"
                   "  {\n"
                   "    if (samplePointSet.samplePoints[i].volumeIndex == -1) continue;\n"
                   "    float t = samplePointSet.samplePoints[i].t;\n"
                   "    if (t < minT)\n"
                   "    {\n"
                   "      minT = t;\n"
                   "      samplePointSet.currentIndex = i;\n"
                   "    }\n"
                   "  }\n"
                   "}\n"
                   "\n"
    ...
    toShaderStr << "bool insertSamplePoint(inout SamplePointSet samplePointSet, in SamplePoint samplePoint)\n"
                   "{\n"
                   "  int index = 2 * samplePoint.volumeIndex + int(samplePoint.type == TYPE_REGION);\n"
                   "  samplePointSet.samplePoints[index] = samplePoint;\n"
                   "  updateMinSamplePoint(samplePointSet);\n"
                   "  return true;\n"
                   "}\n"
                   "\n"
                   ...
                   "  bool popped = false;\n"
                   "\n"
                   "  int index = samplePointSet.currentIndex;\n"
                   "  if (index != -1)\n"
                   "  {\n"
                   "    popped = true;\n"
                   "    front = samplePointSet.samplePoints[index];\n"
                   "    samplePointSet.samplePoints[index].volumeIndex = -1;\n"
                   "    samplePointSet.samplePoints[index].t = FLOAT_MAX;\n"
                   "    updateMinSamplePoint(samplePointSet);\n"
                   "  }\n"
                   "\n"
                   "  return popped;\n"
        ...
        str += "\n"
            ...
            if (bitRegion.mask)
            {
                str +=
                    ...
                    "        vec3 gridCorner = cs * round((hitPoint - .5 * cs) / cs) + .5 * cs; // we move the enlarged volume by half voxel to properly work with rounding (necessary when working with half voxel offset), then restore the original grid corner position\n"
                    "        vec3 nextGridLine = gridCorner + g_rayDirSign * cs;\n"
                    "        vec3 samplePos = gridCorner + g_rayDirSign * .5 * cs; // sampling at the voxel center\n"
                    "\n"
                    "        segDataPos = (in_inverseTextureDatasetMatrix * vec4(samplePos, 1.)).xyz;\n" // TODO these texture matrices might differ for regions
                    "        segTMax = vec3(g_rayDir.x != 0. ? ((nextGridLine.x - g_eyePosObj.x) / g_rayDir.x) : FLOAT_MAX,\n"
                    "                       g_rayDir.y != 0. ? ((nextGridLine.y - g_eyePosObj.y) / g_rayDir.y) : FLOAT_MAX,\n"
                    "                       g_rayDir.z != 0. ? ((nextGridLine.z - g_eyePosObj.z) / g_rayDir.z) : FLOAT_MAX);\n"
                    "        vec3 segTMin = vec3(g_rayDir.x != 0. ? ((gridCorner.x - g_eyePosObj.x) / g_rayDir.x) : FLOAT_MAX,\n"
                    "                            g_rayDir.y != 0. ? ((gridCorner.y - g_eyePosObj.y) / g_rayDir.y) : FLOAT_MAX,\n"
                    "                            g_rayDir.z != 0. ? ((gridCorner.z - g_eyePosObj.z) / g_rayDir.z) : FLOAT_MAX);\n"
                    "        segT = max(segTMin.x, max(segTMin.y, segTMin.z));\n"
                    "        segNormal = -g_rayDirSign * vec3(lessThanEqual(segTMin, vec3(segT)));\n"
                    "        ++stepCounter;\n"
                    "      }\n"
                    "\n"
                  ;
                                 ...
                                 int independentComponents = 0)
  {
    ...
    shaderStr += "\n"
        "      g_srcColor = vec4(0.);\n"
        "      bool computeFragColor = false;\n"
        "      if (doingVol) //if (frontSamplePoint.type == TYPE_VOLUME)\n"
        "      {\n"
    ...
    if (mapper->GetBlendMode() == vtkVolumeMapper::COMPOSITE_BLEND)
    {
      ...
      else
      {
        ...
        if (bitRegion.mask)
        {
            ...
            "        if (g_srcColor.a > 0.)\n"
            "        {\n"
            "          g_srcColor = computeLighting(0, g_srcColor, vec4(segNormal, 1.), TYPE_REGION);\n"
            "          g_srcColor.rgb *= g_srcColor.a;\n"
            "          g_fragColor += (1. - g_fragColor.a) * g_srcColor;\n"
            "          vec3 cs = volumeParameters.data[0].cellSpacing.xyz;\n"
            ...
            "            if (segNormal.x != 0.)\n"
            "            {\n"
            "              if (g_srcColor.a > 0. && (dot(diffBl.yz, vec2(1., 0.)) < dist || dot(diffBl.yz, vec2(0., 1.)) < dist || dot(diffTr.yz, vec2(-1., 0.)) < dist || dot(diffTr.yz, vec2(0., -1.)) < dist))\n"
            "              {\n"
            "                g_fragColor.a = 1.;\n"
            "                g_fragColor.rgb = vec3(.25);\n"
            "              }\n"
            "            }\n"
            "            else if (segNormal.y != 0.)\n"
            "            {\n"
            "              if (g_srcColor.a > 0. && (dot(diffBl.xz, vec2(1., 0.)) < dist || dot(diffBl.xz, vec2(0., 1.)) < dist || dot(diffTr.xz, vec2(-1., 0.)) < dist || dot(diffTr.xz, vec2(0., -1.)) < dist))\n"
            "              {\n"
            "                g_fragColor.a = 1.;\n"
            "                g_fragColor.rgb = vec3(.25);\n"
            "              }\n"
            ...
            "            else\n"
            "            {\n"
            "              if (g_srcColor.a > 0. && (dot(diffBl.xy, vec2(1., 0.)) < dist || dot(diffBl.xy, vec2(0., 1.)) < dist || dot(diffTr.xy, vec2(-1., 0.)) < dist || dot(diffTr.xy, vec2(0., -1.)) < dist))\n"
            "              {\n"
            "                g_fragColor.a = 1.;\n"
            "                g_fragColor.rgb = vec3(.25);\n"
            "              }\n"
