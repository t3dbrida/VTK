...
namespace vtkvolume
{
                                      ...
                                      int independentComponents)
  {
    ...
    toShaderStr << "\n"
                   ...
                   "    int index0 = 2 * i,\n"
                   "        index1 = index0 + 1;\n"
                   "    marks[index0].t = intervals[i].start;\n"
                   "    marks[index0].volumeIndex = i;\n"
                   "    marks[index0].start = true;\n"
                   "    marks[index1].t = intervals[i].end;\n"
                          ...
                          vtkVolume* vtkNotUsed(vol))
  {
    ...
    if (numInputs > 1)
    {
        str += "\n"
            ...
            "    vec2 interval = intersectRayBox((transform * g_eyePosObj).xyz, (transform * vec4(g_rayDir, 0.)).xyz, in_boundsMin[i], in_boundsMax[i]);\n"
            "    intervals[i].start = interval.x;\n"
            "    intervals[i].end = interval.y;\n"
      ...
      default:
      {
        ...
        for (auto& item : inputs)
        {
          ...
          toShaderStr <<
              ...
              "      if (intervals[" << i << "].valid == true && t >= intervals[" << i << "].start && t <= intervals[" << i << "].end)\n"
              "      {\n"
              "        g_skip = false;\n"
                          ...
                          vtkVolume* vtkNotUsed(vol))
  {
    ...
    if (inputCount > 1)
    {
      ...
      str += "\n"
        ...
        "      if (intervals[i].valid == true && t >= intervals[i].start && t <= intervals[i].end)\n"
        "      {\n"
        "        float intervalRange = intervals[i].end - intervals[i].start;\n"
        "        if (intervalRange >= 0. && intervalRange < shortestIntervalRange)\n"
        "        {\n"
        "          shortestIntervalRange = intervalRange;\n"
        "          idx = i;\n"
        "        }\n"
        "      }\n"
        "    }\n"
        ...
        "    if (idx >= 0)\n"
        "    {\n"
        "      g_dataPos += g_dirSteps[idx];\n"
        "      t = dot((in_textureDatasetMatrix[0] * vec4(g_dataPos, 1.) - g_eyePosObj).xyz, g_rayDir) / g_rayDirDot;\n"
        "    }\n"
        "    else\n"
        "    {\n"
        "      for (int i = 0; i < " + inputCount2xStr + "; ++i)\n"
        "      {\n"
        "        if (marks[i].start == true && t < marks[i].t)\n"
        "        {\n"
        "          t = marks[i].t;\n"
        "          idx = marks[i].volumeIndex;\n"
        "          break;\n"
        "        }\n"
        "      }\n"
        "      if (idx >= 0)\n"
        "      {\n"
        "        g_dataPos = (ip_inverseTextureDataAdjusted * vec4(g_eyePosObj.xyz + t * g_rayDir, 1.)).xyz + g_rayJitter[idx + 1];\n"
        "      }\n"
        "      else\n"
        "      {\n"
        "        t = tMax;\n"
