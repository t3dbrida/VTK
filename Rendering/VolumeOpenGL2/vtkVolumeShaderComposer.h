...
namespace vtkvolume
{
                                    ...
                                    const std::map<vtkVolume*, vtkSmartPointer<vtkVolumeTexture>>& masks)
  {
    ...
    toShaderStr <<
      ...
      "    for (int ii = 0; ii < " + std::to_string(inputs.size()) + "; ++ii)\n"
      "    {\n"
      "      if (t[ii] != FLOAT_MAX && t[ii] < minT)\n"
      "      {\n"
      "        minT = t[ii];\n"
      "        i = ii;\n"
      "      }\n"
      ...
      "    if (minT != FLOAT_MAX && g_terminatePosLength >= length((g_eyePosObj.xyz + minT * g_rayDir.xyz) - g_eyePosObj.xyz) && all(lessThanEqual(texPos, volumeParameters.data[i].texMax.xyz)) && all(greaterThanEqual(texPos, volumeParameters.data[i].texMin.xyz)))\n"
      "    {\n"
    ;
      ...
      default:
      {
        ...
        //for (auto& item : inputs)
        {
          ...
          toShaderStr <<
              ...
              "        }\n"
              "\n"
              "        vec3 p = vec3(volumeParameters.data[i].inverseVolumeMatrix * in_volumeMatrix * in_textureDatasetMatrix * vec4(g_dataPos.xyz, 1.)); \n"
              ...
              "    if (minT != FLOAT_MAX)\n"
              "    {\n"
              "      dataPos[i] = dataPos[i] + g_dirSteps[i];\n"
              "      t[i] = dot((in_textureDatasetMatrix * vec4(dataPos[i], 1.) - g_eyePosObj).xyz, g_rayDir) / g_rayDirDot;\n"
              "      if (t[i] > intervals[i].tExit + FLOAT_EPS)\n"
              "      {\n"
              "          t[i] = FLOAT_MAX;\n"
              "      }\n"
              "    }\n"
