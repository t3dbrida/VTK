...
void vtkVolumeTexture::ComputeBounds(VolumeBlock* block)
{
  ...
  block->CellStep[0] = (spacing[0] / static_cast<float>(block->LoadedBounds[1] - block->LoadedBounds[0]));
  block->CellStep[1] = (spacing[1] / static_cast<float>(block->LoadedBounds[3] - block->LoadedBounds[2]));
  block->CellStep[2] = (spacing[2] / static_cast<float>(block->LoadedBounds[5] - block->LoadedBounds[4]));
