...
{
  // original pixel
  float fbdepth = texture(depth,texCoord).r;
  fbdepth = 2.0*nearC/(farC + nearC -fbdepth*(farC - nearC));
  vec4  fbcolor = texture(source,texCoord);
  ...
  for (int i = -3; i <= 3; i++)
    ...
    for (int j = -3; j <= 3; j++)
      {
      float adepth = texture(depth,texCoord + pixelToTCoord*vec2(i,j)).r;
      ...
      if (mdepth < fbdepth*CandidatePointRatio && (i != 0 || j != 0))
        ...
        count = count + 1;
        closestColor += texture(source,texCoord + pixelToTCoord*vec2(i,j));
        closestDepth += adepth;
        }
      }
    }
