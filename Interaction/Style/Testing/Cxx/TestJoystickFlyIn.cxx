...
int TestJoystickFlyIn(int argc, char* argv[])
{
  ...
  // It is also not feasible to test the camera parameters after the fly-in, because they vary based
  // on frame rate as well. Instead, we just run the fly-in and ensure that it completes without
  // crashing. The helper will automatically send a release event after 200 ticks, which should be
  // sufficient for the fly-in to complete.
  bool success = true;
  if (observer->GetFramesSinceLeftButtonDown() > maxFramesBeforeFlyInCompletes)
  {
    std::cerr << "Fly-in did not complete within expected number of frames." << std::endl;
    success = false;
  }
