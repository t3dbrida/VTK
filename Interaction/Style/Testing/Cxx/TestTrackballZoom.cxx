...
int TestTrackballZoom(int argc, char* argv[])
{
  ...
  vtkNew<vtkTrackballZoom> manipulator;
  ...
  {
    interactor->SetEventInformation(i, i, 0, 0, 0, 0);
    interactor->InvokeEvent(vtkCommand::MouseMoveEvent);
  }
  interactor->InvokeEvent(vtkCommand::LeftButtonReleaseEvent);
  renderWindow->Render();

  expectedFocalPoint = { 0.150135, -0.00152028, -2.13825 };
  camera->GetFocalPoint(actualFocalPoint.data());
  success &= assertTuplesEqual(expectedFocalPoint, actualFocalPoint, "focal point after zoom");
  expectedPosition = { 0.150135, -0.00152028, 3.65156 };
  camera->GetPosition(actualPosition.data());
  success &= assertTuplesEqual(expectedPosition, actualPosition, "position after zoom");
  expectedViewUp = { 0.0, 1.0, 0.0 };
  camera->GetViewUp(actualViewUp.data());
  success &= assertTuplesEqual(expectedViewUp, actualViewUp, "view up after zoom");
